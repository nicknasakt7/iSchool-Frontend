...
export default function StudentTeacherComments({
  comments,
  term,
  year,
}: Props) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg text-muted-foreground bg-muted px-2 py-0.5 rounded-full">
            เทอม {term}/{year}
          </span>
          {comments.length > 0 && (
        ...
        <button className="flex items-center gap-1.5 text-md text-muted-foreground hover:text-foreground border border-border rounded-lg px-3 py-1.5 transition-colors">
      ...
      {comments.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 ">
          {comments.map(comment => (
            <div
              key={comment.id}
              className="bg-card border-2 border-primary shadow rounded-2xl p-5 space-y-3 hover:shadow-sm transition-shadow"
            >
              {/* Teacher info */}
              <div className="flex items-center gap-2.5 ">
                ...
                <div>
                  <p className="font-semibold text-md text-foreground leading-tight">
